...
for root, dirs, files in os.walk(src):
    for file in files:
        if "." in file:
            ...
            elif suffix in ("rtf", "doc","docx", "odt", "pdf", 'md', "html" , "css", "js"):
                year_dst = os.path.join(dst , year)
            ...
            elif suffix in ("sql" , "sqlite3" , "sqlite", "db"):
                year_dst = os.path.join(dst , year)
                excel_dest = os.path.join(year_dst, "data base")
                os.makedirs(excel_dest, exist_ok=True)
                os.makedirs(year_dst, exist_ok=True)
                file_dst = os.path.join(year_dst, "data base", file)
                with open(file_root , "rb") as src_file, open(file_dst, "wb") as dst_file:
                    data = src_file.read()
                    dst_file.write(data)

            elif suffix in ("zip" , "rar" , "tar", "gz" , "gz2"):
                year_dst = os.path.join(dst , year)
                excel_dest = os.path.join(year_dst, "zip file")
                os.makedirs(excel_dest, exist_ok=True)
                os.makedirs(year_dst, exist_ok=True)
                file_dst = os.path.join(year_dst, "zip file", file)
                with open(file_root , "rb") as src_file, open(file_dst, "wb") as dst_file:
                    data = src_file.read()
                    dst_file.write(data)
            else:
